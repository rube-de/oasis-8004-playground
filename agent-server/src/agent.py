"""Agent orchestrator for ERC-8004 agent server.
...
class Agent:
    ...
    def ensure_registered(self) -> int:
        ...
        if not self.has_domain:
            raise AgentError(
                "Domain not set. Call set_domain() or use POST /api/config/domain before registration."
            )
    ...
    def _start_api_server(self) -> None:
        ...
        try:
            ...
            # Create thread for API server (pass web3_utility for signing and agent for lifecycle)
            self.api_server_thread = threading.Thread(
                target=api_server.run_server,
                args=("0.0.0.0", port, self.web3_utility, self),
                daemon=True,
                name="api-server"
            )
